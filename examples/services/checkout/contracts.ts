// generated by axon from checkout.toml — do not edit
...
/** The `conversion` view, in `view_conversion`. One method per declared event:
...
export const declaredErrors = {
  checkout: [
    { code: "saga_stuck", status: 500, retriable: false, detail: "the compensation did not get through; the sweep takes it from here" },
  ],
} as const;
...
export function problem(err: unknown, e?: Envelope<unknown>) {
  ...
  return {
    type: p ? `about:axon/checkout/${p.code}` : "about:blank",
...
export class Clients {
   ...
   *  declares: refund_rejected (503, retriable) */
  async paymentsRefundPayment(input: PaymentsRefundPaymentIn, e: Envelope<unknown>): Promise<PaymentsRefundPaymentOut> {
    const attempt = () => withPolicy("payments.refundPayment", { timeoutMs: 8000, retries: 3, breaker: true }, async () =>
      ...
      (code) => ["refund_rejected"].includes(code));
    return attempt();
  }
  /** payments.payoutMerchant · timeout 4000ms · 2 retries · breaker true
   *  declares: merchant_ceiling (409) · rail_busy (503, retriable) */
