...
--   axon rls manifests/ --service payments > sql-policies/payments/R__rls.sql
--
...
-- payments.inbox_seen: axon's own plumbing. No policy —it holds no tenant's
-- rows— and the grant is not optional: the application writes it in
-- the same transaction as the change it belongs to.
GRANT SELECT, INSERT, UPDATE, DELETE ON "inbox_seen" TO axon_app;

-- payments.outbox: axon's own plumbing. No policy —it holds no tenant's
-- rows— and the grant is not optional: the application writes it in
-- the same transaction as the change it belongs to.
GRANT SELECT, INSERT, UPDATE, DELETE ON "outbox" TO axon_app;
