...
the whole system up and makes **72 checks against reality**:
...
It runs in CI on every push. The whole run —28 sections, 72 checks— and what happens when
...
| --- | --- | --- |
| **Docker** | `--target local`: broker, Postgres per service, MinIO, Jaeger, flagd, the edge and your services | `./demo.sh` brings the system up and makes 72 checks against reality |
