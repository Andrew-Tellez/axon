...
the whole system up and makes **65 checks against reality**:
...
It runs in CI on every push. The whole run —26 sections, 65 checks— and what happens when
...
| --- | --- | --- |
| **Docker** | `--target local`: broker, Postgres per service, MinIO, Jaeger, flagd, the edge and your services | `./demo.sh` brings the system up and makes 65 checks against reality |
...
| [The demo, measured](https://andrew-tellez.github.io/axon/demo.html) | The 65 checks against real containers, and what running it twice proves |
