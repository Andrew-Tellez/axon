...
fn pooler_config(ms: &[Manifest], svc: &str) -> Option<PoolerConfig> {
    ...
    Some(PoolerConfig { pgdog, users, vars })
}
...
pub fn render(p: &Plan, target: &str) -> Result<String, String> {
    ...
    if matches!(target, "gcp" | "aws") {
        if let Some(s) = p.stores.iter().find(|s| s.shards.unwrap_or(1) > 1) {
            return Err(format!(
                "{}: `[pooler] shards = {}` is not rendered on `{target}` yet. Today the sharder \
                 comes up on `--target local` and on `--target k8s`; emitting a single instance \
                 here would apply with no error and leave the sharding non-existent. \
                 `--target plan` gives the plan with the nodes and the pooler's configuration \
                 so you can render it with your own template.",
                s.service,
