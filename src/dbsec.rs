...
/// The policies for every service, or for one of them.
///
/// `only` exists because the compose mounts ONE directory per service —
/// `sql-policies/<service>/` — and this used to emit every service into a
/// single file. Whoever followed the header's own instruction got every
/// service's policies applied to the first service's database.
pub fn build(ms: &[Manifest], only: Option<&str>) -> String {
    let ms: Vec<Manifest> = ms
        .iter()
    ...
    let mut o = vec![
        ...
        format!(
            "--   axon rls manifests/ --service {} > sql-policies/{}/R__rls.sql",
            only.unwrap_or("<service>"),
            only.unwrap_or("<service>")
        ),
        "--".to_string(),
