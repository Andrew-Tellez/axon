...
enum Cmd {
    ...
    /// the Temporal workflows, as their OWN module. They are not part of
    /// `axon build` for a reason that is Temporal's and not a preference: the
    /// worker loads workflow code in an isolated bundle, so it cannot live in
    /// the same file as the transport, the clients and the database types
    Workflows {
        #[arg(value_hint = clap::ValueHint::AnyPath)]
        manifest: String,
        /// directory with the others; that is where a consumed event's type
        /// comes from
...
fn run() -> Result<ExitCode, String> {
    ...
    match cli.cmd {
        ...
        Cmd::Workflows { manifest, peers } => {
            let m = manifest::load_any(&manifest)?;
            let all = manifest::discover(&[peers])?;
            if !m.workflow.values().any(|w| w.engine.temporal()) {
                return Err(format!(
                    "{}: no `[workflow]` with `engine = \"temporal\"`. The ones on the saga \
                     engine are generated by `axon build`, with the rest of the service",
                    m.service
                ));
            }
