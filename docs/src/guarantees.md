...
| The declared limit | the demo ramps past it against the real edge: it throttles with 429 and does not break with 5xx |
...
| The real chain from spans | the demo reads the running Jaeger and checks the edge it just produced is there and declared, and that an undeclared one fails |
...
cd examples && ./demo.sh  # 65 checks against real containers
```
