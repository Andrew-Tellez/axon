...
### The drift the other way

axon emits the questions; the ones somebody writes **by hand** in the dashboard, against a
table axon owns, were invisible to it. The day the column changes, that question breaks
and nothing says so until somebody opens it.

`axon analytics <sources> --metabase --check <export.json>` reads what came back from the
Metabase —its own `/api/card` export, or the shape axon emits— and crosses it against the
schema it generates, read from the DDL itself and not from a second list:

```console
$ axon analytics manifests/ --metabase --check cards.json
42 question(s) exported  ·  4 generated by axon  ·  38 written by hand, 37 of them not checkable
  quien compro (a mano)  ·  order_placed_v1  ·  reads customer_email
warn  37 question(s) are not native and are NOT checked: they name their table and their fields by numeric id, and from outside the Metabase those ids say nothing
error `quien compro (a mano)`: reads `order_placed_v1.customer_email`, and that column is not in what the manifest generates —what is there is `customer_email_hash`
```

No credentials here either: whoever has the Metabase exports its questions and the
compiler does the diff.

The commonest failure is not a typo. It is the **PII column**: the manifest says that
field travels hashed, so it never exists by that name, and a question asking for the
address in plaintext answers with an error forever. Saying which column *is* there is what
turns the error into the fix.

And what it refuses to answer: a question that is not native names its table and its
fields by numeric id, and from outside the Metabase those ids say nothing; one that joins
or reads a CTE makes a bare name ambiguous. Both are counted and named as **not checked**
— never quietly passed. They come out as one line each and not one per question, because a
Metabase ships with dozens of example questions of its own and forty warnings about
somebody else's sample database is how a rule stops being read.

## What is missing

A native generator beyond TypeScript, and a storage engine beyond the one Postgres knows.
Sharding on `gcp` and `aws`, where the nodes are N managed instances. And declared
scenarios, which need fault injection before they mean anything.
